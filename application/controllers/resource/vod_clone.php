...
class vod_clone extends CI_Controller {
    ...
    function index() {
        ...
        $data['title'] = 'จัดการวิดีโอ VOD clone';
        ...
        $data['clone_link'] = site_url('resource/vod_clone/clone_all');
        if ($this->make_money) {
        ...
        $this->template->script_var(array(
            'ajax_grid_url' => site_url('resource/vod_clone/ajax_list'),
                //  'ajax_act_resource_url' => site_url('resource/vod_clone/ajax_act_resource')
        ));
        $data['main_side_menu'] = $this->main_side_menu_model->resource('vod_clone');
        $this->template->application_script('resource/vod_clone/main_grid.js');
        $this->template->load_showloading();
        $this->template->load_flexgrid();
        $this->template->write_view('resource/vod_clone/main_grid', $data);
    ...
    function ajax_act_resource() {
        ...
        if ($a_resource_id) {
            switch ($this->input->post('command')) {
                case 'to_delete':
                    $a_result['status'] = $this->vod_clone_model->delete($a_resource_id);
                    break;
                case 'to_private':
                    $a_result['status'] = $this->vod_clone_model->privacy($a_resource_id, 0);
                    break;
                case 'to_no_private':
                    $a_result['status'] = $this->vod_clone_model->privacy($a_resource_id, 1);
                    break;
                case 'to_publish':
                    $a_result['status'] = $this->vod_clone_model->publish($a_resource_id, 1);
                    break;
                case 'to_no_publish':
                    $a_result['status'] = $this->vod_clone_model->publish($a_resource_id, 0);
                    break;
                default:
                    break;
            }
    ...
    function ajax_list() {
        $a = $this->vod_clone_model->find_all($this->input->post('page', TRUE), $this->input->post('qtype', TRUE), $this->input->post('query'), $this->input->post('rp', TRUE), $this->input->post('sortname', TRUE), $this->input->post('sortorder', TRUE));
        echo json_encode($a);
    }
    function clone_all(){
        $this->vod_clone_model->clone_all();
    }
    ...
    function edit($resource_id) {
        $url = site_url('resource/vod_clone');
        if ($this->vod_clone_model->get_resource_type_id($resource_id) == 6) {
            $url = site_url('resource/vod_clone/prokru');
        }
        if (!$this->vod_clone_model->is_owner($resource_id)) {
            $data = array(
                'time' => 1,
                'url' => $url,
                'heading' => 'ระบบผิดพลาด',
            ...
            $this->load->helper('form');
            $this->template->load_showloading();
            $this->template->application_script('resource/vod_clone/edit_form.js');

            $data = array(
                'title' => 'แก้ไขรายละเอียดวีดีโอ',
                'form_action' => site_url('resource/vod_clone/do_save'),
                'form_data' => $this->vod_clone_model->get_video_form_data($resource_id),
                'cancel_link' => $url,
                'delete_link' => site_url('resource/vod_clone/delete/' . $resource_id),
            ...
            $this->template->write_view('resource/vod_clone/edit_form', $data);
            $this->template->render();
        }
    }

    function do_save() {
        if ($this->vod_clone_model->save($this->input->post('data'))) {
            $data = array(
                'time' => 1,
                'url' => site_url('resource/vod_clone'),
    ...
    function delete($resource_id) {
        $data = array(
            'url' => site_url('resource/vod_clone/do_delete/' . $resource_id),
            'cancel_url' => site_url('resource/vod_clone'),
    ...
    function do_delete($id) {
        if ($this->vod_clone_model->delete($id)) {
            $data = array(
                'time' => 1,
                'url' => site_url('resource/vod_clone'),
    ...
    function update_duration() {
        $this->vod_clone_model->update_all_duration();
        //exit();
        $data = array(
            'time' => 1,
            'url' => site_url('resource/vod_clone'),
    ...
    function prokru() {
        ...
        $this->vod_clone_model->set_resource_type_id(6);
        $this->template->load_showloading();
        $this->template->load_flexgrid();
        $this->template->application_script('resource/vod_clone/main_grid_prokru.js');
        $data['title'] = 'จัดการ วิดีโอจาก prokru';
        $data['add_link'] = site_url('resource/vod_clone/add_prokru');

        $data['main_side_menu'] = $this->main_side_menu_model->resource('vod_clone');

        $data['qtype_options'] = array('title' => 'ชื่อวิดีโอ', 'resource_id' => 'เลขที่วิดีโอ', 'desc' => 'รายละเอียด', 'tags' => 'tags', 'category_id' => 'หมวดหมู่');
        $data['ajax_grid_url'] = site_url('resource/vod_clone/ajax_prokru_uploads_list');
        $this->template->write_view('resource/vod_clone/main_grid_prokru', $data);
        $this->template->render();
    }

    function ajax_prokru_uploads_list() {
        $this->vod_clone_model->set_resource_type_id(6);
        $a = $this->vod_clone_model->find_all($this->input->post('page', TRUE), $this->input->post('qtype', TRUE), $this->input->post('query', TRUE), $this->input->post('rp', TRUE), $this->input->post('sortname', TRUE), $this->input->post('sortorder', TRUE));
        echo json_encode($a);
    }
    ...
    function add_prokru() {
        ...
        $data = array(
            ...
            'form_action' => site_url('resource/vod_clone/do_save_prokru'),
            'form_data' => $this->vod_clone_model->get_video_form_data(),
            'cancel_link' => site_url('resource/vod_clone/prokru'),
            'publish_options' => $this->ddoption_model->get_publish_options(),
            'privacy_options' => $this->ddoption_model->get_privacy_options(),
            ...
            'ajax_prokru_get_video_data_url' => site_url('resource/vod_clone/ajax_prokru_get_video_data')
        );
        $this->template->write_view('resource/vod_clone/prokru_add_form', $data);
        $this->template->render();
    }
    ...
    function edit_prokru() {
        if (!$this->vod_clone_model->is_owner($resource_id)) {
            ...
            $data = array(
                'form_action' => site_url('resource/vod_clone/do_save'),
                'form_data' => $this->vod_clone_model->get_video_form_data($resource_id),
                'cancel_link' => site_url('resource/vod_clone'),
                'delete_link' => site_url('resource/vod_clone/delete/' . $resource_id),
                'publish_options' => $this->ddoption_model->get_publish_options(),
                'privacy_options' => $this->ddoption_model->get_privacy_options(),
...
//            5 => 'โจทย์เติมคำหลายคำตอบ(ctma)'
        ...
        $this->template->script_var(array(
            'ajax_grid_url' => site_url('resource/vod_clone/ajax_iframe_list/'),
            'input_id' => $input_id
        ));

        if ($this->make_money) {
            $this->template->application_script('resource/vod_clone/iframe_main_grid.js');
        } else {
            $this->template->application_script('resource/vod_clone/iframe_main_grid_freesys.js');
        }

        $this->template->load_showloading();
        $this->template->load_flexgrid();
        $this->template->write_view('resource/vod_clone/iframe_grid', $data);
        $this->template->temmplate_name('normal');
        $this->template->render();
    }

    function ajax_iframe_list() {
        $a = $this->vod_clone_model->iframe_find_all($this->input->post('page', TRUE), $this->input->post('qtype', TRUE), $this->input->post('query', TRUE), $this->input->post('rp', TRUE), $this->input->post('sortname', TRUE), $this->input->post('sortorder', TRUE));
        echo json_encode($a);
    }
