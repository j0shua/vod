...
//====================================================================================================================
    ...
    function add_course_act($c_id) {
        ...
        $st_id_options_normal_sheet_set = $st_id_options_test_no_online = $st_id_options_test = $st_id_options_normal_no_online = $st_id_options_normal;
        unset($st_id_options_normal_sheet_set[1]);
        unset($st_id_options_normal_sheet_set[2]);
        unset($st_id_options_normal_sheet_set[3]);
        unset($st_id_options_normal_sheet_set[4]);
        unset($st_id_options_normal_no_online[4]);
        unset($st_id_options_test[5]);
        unset($st_id_options_test_no_online[5]);
        unset($st_id_options_test_no_online[4]);
        ...
        //for ไม่ทำออนไลน์
        $tmp_option = array();
        foreach ($st_id_options_normal_no_online as $k => $v) {
            $tmp_option[] = "'" . $k . "' : '" . $v . "'";
        }
        $st_id_options_normal_no_online = '{' . implode(',', $tmp_option) . '}';

        //for ทำข้อสอบ
        $tmp_option = array();
        foreach ($st_id_options_test as $k => $v) {
            $tmp_option[] = "'" . $k . "' : '" . $v . "'";
        }
        $st_id_options_test = '{' . implode(',', $tmp_option) . '}';

        //for ทำข้อสอบ ไม่ทำออนไลน์
        $tmp_option = array();
        foreach ($st_id_options_test_no_online as $k => $v) {
            $tmp_option[] = "'" . $k . "' : '" . $v . "'";
        }
        $st_id_options_test_no_online = '{' . implode(',', $tmp_option) . '}';

        //for ชุดการสอน
        $tmp_option = array();
        foreach ($st_id_options_normal_sheet_set as $k => $v) {
            $tmp_option[] = "'" . $k . "' : '" . $v . "'";
        }
        $st_id_options_normal_sheet_set = '{' . implode(',', $tmp_option) . '}';

        $this->template->script_var(array(
            'sheet_browser_iframe_url' => site_url('resource/sheet/iframe/data'),
            'st_id_options_normal' => array('value' => $st_id_options_normal),
            'st_id_options_normal_no_online' => array('value' => $st_id_options_normal_no_online),
            'st_id_options_test' => array('value' => $st_id_options_test),
            'st_id_options_test_no_online' => array('value' => $st_id_options_test_no_online),
            'st_id_options_normal_sheet_set' => array('value' => $st_id_options_normal_sheet_set),
            'st_id' => 2,
...
//        );
    ...
    function ranking_ptest($ca_id) {
        $this->load->model('study/ptest_model');
        $this->ptest_model->processing_summary($ca_id);
        $course_act_data = $this->course_model->get_course_act_data($ca_id);
        $ranking_pre_data = $this->ptest_model->get_ranking($ca_id, 'pre');

        if ($ranking_pre_data) {
            $data['title'] = 'คะแนน พรีเทส โพสเทส ของ '.$course_act_data['title'];
            $data['ranking_pre_data'] = $ranking_pre_data;

            $data['ranking_post_data'] = $this->ptest_model->get_ranking($ca_id, 'post');
            $this->template->write_view('study/course_manager/ptest_ranking', $data);
            $this->template->render();
        } else {
            $data = array(
                'time' => 5,
...
//    function processing_summary_ptest() {
//        $this->load->model('study/ptest_model');
//        $this->ptest_model->processing_summary();
//    }
