...
class manual_topup extends CI_Controller {
    ...
    function index() {
        ...
        $data['grid_menu'] = array(
            array('url' => site_url('utopup/manual_topup/quick_topup'), 'title' => 'เติมเงินแบบเร็ว', 'extra' => '')
        );
        $data['main_side_menu'] = $this->topup_menu_model->main_side_menu('topup_manager');
        $this->template->script_var(array(
            'ajax_grid_url' => site_url('utopup/manual_topup/ajax_topup_list')
        ));
    ...
    function informant_manager() {
        ...
        $this->template->script_var(array(
            'ajax_grid_url' => site_url('utopup/manual_topup/ajax_informant_list')
        ));
    ...
    function inform_transfer() {
        ...
        $data['form_action'] = site_url('utopup/manual_topup/to_inform_transfer');
        ...
        $this->template->script_var(array(
            'ajax_search_user_url' => site_url('core/ajax_search/user')
        ));
...
//        $this->manual_topup_model->inform_transfer($post);
    ...
    function quick_topup() {
        ...
        $data['form_action'] = site_url('utopup/manual_topup/to_quick_topup');
        $data['cancel_link'] = site_url('utopup/manual_topup');
        $this->template->script_var(array(
            'ajax_search_user_url' => site_url('core/ajax_search/user')
        ));
        $this->template->load_jquery_ui_timepicker();
    ...
    function to_quick_topup() {
        ...
        $data = array(
            'time' => 5,
            'url' => site_url('utopup/manual_topup'),
    ...
    function infrom_topup($mt_id) {
        ...
        $data['form_action'] = site_url('utopup/manual_topup/to_infrom_topup');
        $data['cancel_link'] = site_url('utopup/manual_topup');
    ...
    function no_transfer($mt_id) {
        $this->auth->access_limit($this->auth->permis_topup_manager);
        $data['inform_data'] = $this->manual_topup_model->get_inform_data($mt_id);
        $data['form_action'] = site_url('utopup/manual_topup/do_no_transfer');
        $data['cancel_link'] = site_url('utopup/manual_topup');
