...
//            array('url' => site_url('study/course_manager/add_course'), 'title' => 'เพิ่มหลักสูตร', 'extra' => ''),
       ...
       // $data['main_side_menu'] = $this->main_side_menu_model->study('student_course_report');
        $this->template->script_var(
                array('ajax_grid_url' => site_url('report/student_report/ajax_course'))
        );
        $this->template->application_script('report/student_report/course_report_grid.js');
        $this->template->load_showloading();
        $this->template->load_flexgrid();
        $this->template->write_view('report/student_report/course_report_grid', $data);
    ...
    function ajax_course() {
        $a = $this->course_model->find_all_my_course_rp($this->input->post('page', TRUE), $this->input->post('qtype', TRUE), $this->input->post('query'), $this->input->post('rp', TRUE), $this->input->post('sortname', TRUE), $this->input->post('sortorder', TRUE));
        echo json_encode($a);
    }
