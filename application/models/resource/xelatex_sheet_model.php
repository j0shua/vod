...
\center{' . $sheet_head_data['title'] . ' ' . $chapter_title . '}\\ ';
    ...
    private function make_dycontent($data, $vspace, $show_solve = FALSE) {
        ...
        if (isset($data['content_questions'])) {
            ...
            foreach ($data['content_questions'] as $question) {
                ...
                switch ($question['content_type_id']) {
                    case 2:
                        ...
                        if ($show_solve) {
                            ...
                            if (isset($question['true_answers'][0])) {
                                //$this->set_solve_formula("ข้อ " . ($this->question_num + 1));
                                $this->set_solve_formula("");
                                $this->set_solve_formula("\\noindent");
                                $this->set_solve_formula('\textbf{' . "ข้อ " . ($this->question_num + 1) . ' ตอบข้อ} ' . $this->itemize_char[$this->itemize_type][($question['true_answers'][0])]);
                                $this->set_solve_formula("\\\\");
                            }
                        }
                        break;
                    ...
                    case 4:
                        ...
                        if (isset($question['true_answers'][0])) {
                                ...
                                $this->set_solve_formula("");
                                $this->set_solve_formula("\\noindent");
                                $this->set_solve_formula('\textbf{' . "ข้อ " . ($this->question_num + 1) . ' ตอบ} \fbox{'. $question['true_answers'][0].'}' );
                                $this->set_solve_formula("\\\\");
                            }
                        break;
                    case 5:
                ...
                if ($show_solve) {
                    $this->set_solve_formula('\textbf{เฉลย}' . "\\\\");
                    $this->set_solve_formula($question['solve_answer']);
                    $this->set_solve_formula('\vspace{' . (2 * $this->multiplier_line) . 'mm}');
                }
    ...
    function set_solve_formula($formula) {
        $this->solve_formula[] = $formula;
    }
