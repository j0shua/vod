...
class practice_model extends CI_Model {
            ...
            if ($sheet_data['data']['question_num'] > 0) {
                ...
                foreach ($sheet_data['data']['sheet_set'] as $k => $q) {
                    ...
                    if (isset($q['resource_id'])) { //ถ้าเป็น dycontent
                        ...
                        if ($dycontent_data['content_type_id'] != 1) {

                            $resource_id_new = '';
                            if ($dycontent_data['num_questions'] > 1) {
                                $resource_id_new = $this->get_resource_id_question_same_sub_chapter($q['resource_id'], $resource_id_not_in, $dycontent_data['num_questions']);
                                if ($resource_id_new) {
                                    $resource_id_not_in[] = $resource_id_new;
                                    $get_single_q = FALSE; //ไม่เจอข้อสอบต่อเนื่อง
                                }
                            }

                            $loops = 1;
                            if ($resource_id_new == '') {
                                $loops = $dycontent_data['num_questions'];
                            }
                            for ($index = 0; $index < $loops; $index++) {
                                ...
                                $inner_k_new = 0;
                                foreach ($dycontent_data_new['data']['content_questions'] as $inner_q_new) {
    ...
    function make_answer_sheet_bk($ca_id, $uid = '') {
        if ($uid == '') {
            $uid = $this->auth->uid();
        }
        $act_data = $this->CI->course_model->get_act_data($ca_id);
    ...
    function get_resource_id_question_same_sub_chapter($resource_id, $resource_id_not_in = array(), $num_questions = 1) {
        $where_not_in = '';
...
where r_resource.resource_id=r_resource_dycontent.resource_id and content_type_id != 1 and num_questions =$num_questions 
ORDER BY RAND()
LIMIT 1
";
...
";
    ...
    function commit_practice_answer_sheet($caasp_id) {
        ...
        $set = array(
            'get_score' => $score_data['get_score'],
            'send_time' => $this->time,
            'update_time' => $this->time,
...
FROM (SELECT * FROM s_course_act_asheet_practice WHERE s_course_act_asheet_practice.ca_id = $ca_id and s_course_act_asheet_practice.send_time > 0)s_course_act_asheet_practice
GROUP BY s_course_act_asheet_practice.uid 
ORDER BY s_course_act_asheet_practice.get_score DESC
";
        $q = $this->db->query($sql);
