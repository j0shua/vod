...
function validateForm() {
    ...
    //	var swfupload_file_name = document.getElementById("swfupload_file_name");
...
function fileQueued(file) {
    try {
//        var swfupload_file_name = document.getElementById("swfupload_file_name");
//        swfupload_file_name.value = file.name;
        $("#title").val(file.name);
    } catch (e) {
    }
...
function uploadProgress(file, bytesLoaded, bytesTotal) {
    ...
    try {
        ...
        progress.setStatus("กำลัง Upload..."+percent+" %");
    } catch (e) {
    }
}

function uploadSuccess(file, serverData_json) {
    try {
        file.id = "singlefile";	// This makes it so FileProgress only makes a single UI element, instead of one for each file
        var progress = new FileProgress(file, this.customSettings.progress_target);
        ...
        var serverData = jQuery.parseJSON(serverData_json);
        console.log(serverData);
        if (serverData.complete) {
            document.getElementById("resume_file").value = serverData.resume_file;
            this.customSettings.upload_successful = true;
        } else {
            alert(serverData.error_msg);
